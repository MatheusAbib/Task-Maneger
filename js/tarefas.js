...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Forçar o datepicker nativo apenas quando clicar no ícone
    dateInput.addEventListener('click', function(e) {
        // Verifica se o clique foi na área do ícone (últimos 40px)
        if (e.clientX > this.getBoundingClientRect().right - 40) {
            this.showPicker(); // Mostra o datepicker nativo
        }
    });
    
    // Formatar a data quando selecionada
    dateInput.addEventListener('change', function() {
        if (this.value) {
            ...
            // Você pode mostrar a data formatada em outro elemento se quiser
        }
    });
});